function [sim_output] = IKK_sim_tiff_measure_noise(tiff_dir,beads_inward)
...
for z = 1:size(beads_inward,2)+1
    ...
    if z < size(beads_inward,2)+1
        ...
        cd(sprintf('%s%stiff_IKK_pericentromere_%s_%d_n',tiff_dir,filesep,tiff_str,beads_inward(z)));
    else
        % go to the tiff folder
