function [protein_map] = heatmap_maker_ver2_rand_perm(data_cat,stationary_color,red_dist_lim,green_dist_lim,desired_n)
...
% randomly choose the plane
chosen_plane = randperm(size(plane,3),desired_n/2);

% preallocate
protein_map = zeros([heatmap_size heatmap_size size(chosen_plane,2)]);

% reassign the planes
for z = 1:size(chosen_plane,2)
    protein_map(:,:,z) = plane(:,:,chosen_plane(z));
end

% sum the planes
protein_map = sum(protein_map,3);
