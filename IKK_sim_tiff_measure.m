function [sim_output] = IKK_sim_tiff_measure(tiff_dir,beads_inward)
...
for z = 1:size(beads_inward,2)+1
    
    if z < size(beads_inward,2)+1
        % go to the tiff folder
        cd(sprintf('%s%stiff_IKK_pericentromere_%s_%d',tiff_dir,filesep,tiff_str,beads_inward(z)));
    else
        % go to the tiff folder
        cd(sprintf('%s%stiff_IKK_pericentromere_%s_MT',tiff_dir,filesep,tiff_str));
    end
    
    % identify all the tif files
    tiff_files = dir('*.tif');
    
    % if this is the first loop, preallocate the structure
    if z == 1
        % preallocate
        sim_output.coords_bp = cell([size(beads_inward,2)+2 7 size(tiff_files,1)]);
        sim_output.coords_gauss = cell([size(beads_inward,2)+2 4 size(tiff_files,1)]);
        ...
        for h = 1:size(tiff_files,1)
            ...
            for y = 1:size(beads_inward,2)
                sim_output.coords_bp{y+1,1,h} = num2str(beads_inward(y));
                sim_output.coords_gauss{y+1,1,h} = num2str(beads_inward(y));
            end
            sim_output.coords_bp{size(beads_inward,2)+2,1,h} = 'MT';
            sim_output.coords_gauss{size(beads_inward,2)+2,1,h} = 'MT';
        end
    else
    end
...
% preallocate
kk_vals_bp = zeros([size(sim_output.coords_bp,3) size(beads_inward,2)+1]);
kk_vals_gauss = zeros([size(sim_output.coords_bp,3) size(beads_inward,2)+1]);
...
% preallocate
sim_output.kk_values = cell([size(beads_inward,2)+2 5]);
sim_output.kk_values(1,:) = {'Bead Idx','KK Mean (bp)','KK StDev (bp)','KK Mean (gauss)','KK StDev (gauss)'};
for z = 1:size(beads_inward,2)
    sim_output.kk_values{z+1,1} = beads_inward(z);
end
sim_output.kk_values{size(beads_inward,2)+2,1} = 'MT';
...
% preallocate
sim_output.DNA_cen_pos_bp = cell([size(beads_inward,2)+2 8]);
...
for z = 1:size(beads_inward,2)
    sim_output.DNA_cen_pos_bp{z+1,1} = beads_inward(z);
end
sim_output.DNA_cen_pos_bp{size(beads_inward,2)+2,1} = 'MT';

% find the bp measurements for DNA-cen measurements
for z = 1:size(beads_inward,2)+1
...
% preallocate
sim_output.DNA_cen_pos_gauss = cell([size(beads_inward,2)+2 8]);
...
for z = 1:size(beads_inward,2)
    sim_output.DNA_cen_pos_gauss{z+1,1} = beads_inward(z);
end
sim_output.DNA_cen_pos_gauss{size(beads_inward,2)+2,1} = 'MT';
...
% preallocate
sim_output.FHWM = cell([size(beads_inward,2)+2 5]);
sim_output.FHWM(1,:) = {'Bead Idx','FWHM-X Mean','FHWM-X StDev','FWHM-Y Mean','FHWM-Y StDev'};
for z = 1:size(beads_inward,2)
    sim_output.FHWM{z+1,1} = beads_inward(z);
end
sim_output.FHWM{size(beads_inward,2)+2,1} = 'MT';

% preallocate
x_FHWM = zeros([size(sim_output.coords_bp,3)*2 size(beads_inward,2)+1]);
y_FHWM = zeros([size(sim_output.coords_bp,3)*2 size(beads_inward,2)+1]);
